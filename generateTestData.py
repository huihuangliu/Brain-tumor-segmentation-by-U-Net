...
train_data_path = "./BRATS-testing/HGG/**/**/*.mha"
...
flies_all_name = [file_names_flair[0:10], file_names_t1[0:10], file_names_t1c[0:10], file_names_t2[0:10]]
...
for time in [1]:
    ...
    for i in range(half[time]):
        ...
        for j in range(all_3d_data[0].shape[0]):
            if seg_img[j, :, :].any():
    ...
    X_train_target = X_train_target[index, :, :]
     '''  
    testimg = X_train_input
    testlab = X_train_target
