...
#        conv1 = DropoutLayer(conv1, keep=0.2, name='drop1')
        ...
        conv2 = BatchNormLayer(pool1, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn1')
        
        conv2 = Conv2d(conv2, 128, (3, 3), act=tf.nn.relu, name='conv2_1')
#        conv2 = DropoutLayer(conv2, keep=0.2, name='drop2')
        conv2 = Conv2d(conv2, 128, (3, 3), act=tf.nn.relu, name='conv2_2')
        pool2 = MaxPool2d(conv2, (2, 2), name='pool2')   #60
        conv3 = BatchNormLayer(pool2, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn2')

        conv3 = Conv2d(conv3, 256, (3, 3), act=tf.nn.relu, name='conv3_1')
#        conv3 = DropoutLayer(conv3, keep=0.2, name='drop3')
        conv3 = Conv2d(conv3, 256, (3, 3), act=tf.nn.relu, name='conv3_2')
        pool3 = MaxPool2d(conv3, (2, 2), name='pool3')    #30
        conv4 = BatchNormLayer(pool3, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn3')
  
        conv4 = Conv2d(conv4, 512, (3, 3), act=tf.nn.relu, name='conv4_1')
#        conv4 = DropoutLayer(conv4, keep=0.2, name='drop4')
        conv4 = Conv2d(conv4, 512, (3, 3), act=tf.nn.relu, name='conv4_2')
        pool4 = MaxPool2d(conv4, (2, 2), name='pool4')   #15
        conv5 = BatchNormLayer(pool4, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn4')
...
#        conv5 = DropoutLayer(conv5, keep=0.2, name='drop5')
        ...
        up4 = DeConv2d(conv5, 512, (3, 3), (nx/8, ny/8), (2, 2), name='deconv4')
        up4 = BatchNormLayer(up4, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn5')
...
#        conv4 = DropoutLayer(conv4, keep=0.2, name='drop6')
        conv4 = Conv2d(conv4, 512, (3, 3), act=tf.nn.relu, name='uconv4_2')
        up3 = DeConv2d(conv4, 256, (3, 3), (nx/4, ny/4), (2, 2), name='deconv3')
        up3 = BatchNormLayer(up3, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn6')        
...
#        conv3 = DropoutLayer(conv3, keep=0.2, name='drop7')
        conv3 = Conv2d(conv3, 256, (3, 3), act=tf.nn.relu, name='uconv3_2')
        up2 = DeConv2d(conv3, 128, (3, 3), (nx/2, ny/2), (2, 2), name='deconv2')
        up2 = BatchNormLayer(up2, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn7')                
...
#        conv2 = DropoutLayer(conv2, keep=0.2, name='drop8')
        conv2 = Conv2d(conv2, 128, (3, 3), act=tf.nn.relu, name='uconv2_2')
        up1 = DeConv2d(conv2, 64, (3, 3), (nx/1, ny/1), (2, 2), name='deconv1')
        up1 = BatchNormLayer(up1, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn8')                               
...
#        conv7 = BatchNormLayer(conv7, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn7')
        #2*2
        conv7 = Conv2d(conv6, 512, (4, 4), (2, 2), act=lambda x: tl.act.lrelu(x, 0.2), padding=pad, W_init=w_init, b_init=b_init, name='conv8')
        print(" * After conv: %s" % conv7.outputs)   #2*2
        # exit()
        # print(nx/8)
        up6 = DeConv2d(conv7, 512, (4, 4), out_size=(2, 2), strides=(2, 2),
                                    padding=pad, act=None, batch_size=batch_size, W_init=w_init, b_init=b_init, name='deconv7')
        up6 = BatchNormLayer(up6, act=tf.nn.relu, is_train=is_train, gamma_init=gamma_init, name='dbn7')
        #4*4
        # print(up6.outputs)
        up6 = ConcatLayer([up6, conv6], 3, name='concat6')
        print('the layer: {}     {}'.format(up6.outputs, conv6))
...
#        up6 = BatchNormLayer(up6, act=tf.nn.relu, is_train=is_train, gamma_init=gamma_init, name='dbn6')
        ...
        #print('the layer: {}     {}'.format(up5.outputs, conv5.outputs))
        up5 = DeConv2d(up6, 1024, (4, 4), out_size=(8, 8), strides=(2, 2),
        ...
        print('the layer: {}     {}'.format(up4.outputs, conv4.outputs))
        ...
        print('the layer: {}     {}'.format(up3.outputs, conv3.outputs))
        ...
        print('the layer: {}     {}'.format(up2.outputs, conv2.outputs))
        ...
        print('the layer: {}     {}'.format(up1.outputs, conv1.outputs))
...
def u_net_bn(x, is_train=False, reuse=False, batch_size=None, pad='SAME', n_out=1):
    ...
    with tf.variable_scope("u_net", reuse=reuse):
        ...
        conv3 = BatchNormLayer(conv3, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn3')
        print('============================')
        print('conv4 layer: {}'.format(conv4.outputs))
        conv4 = Conv2d(conv3, 512, (4, 4), (2, 2), act=None, padding=pad, W_init=w_init, b_init=b_init, name='conv4')
        print('============================')
        print('conv4 layer: {}'.format(conv4.outputs))
        conv4 = BatchNormLayer(conv4, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn4')
        ...
        print('the layer: {}     {}'.format(up7.outputs, conv7))
        up6 = DeConv2d(up6, 1024, (4, 4), out_size=(4, 4), strides=(2, 2),
                                    padding=pad, act=None, batch_size=batch_size, W_init=w_init, b_init=b_init, name='deconv6')
        up6 = BatchNormLayer(up6, act=tf.nn.relu, is_train=is_train, gamma_init=gamma_init, name='dbn6')
        # print(up6.outputs)
        ...
        up5 = ConcatLayer([up6, conv6], 3, name='concat5')
        print('the layer: {}     {}'.format(up6.outputs, conv6.outputs))
